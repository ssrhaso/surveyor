...
# -- lets two/three boxes with a SHARED home split one stage's job list
...
run_arms() {
  ...
  for c in "${!NAMES[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
      cmds+=("echo '=== arm=$NAME seed=$SEED ==='; python -m ffjepa.eval_reacher --subgoal ${SG} ${CKPT_ARG} --accept-tau 0.20 --gdm-steps ${K} --source local --local-dir encoder_reacher --h5 $h5 --device cuda --num-eval 128 --seed ${SEED} --episode-min 8000 --goal-offset 25 --eval-budget 50 --horizon ${RH} --receding-horizon ${RH} --dump-traces runs/reacher/traces_${NAME}_seed${SEED}.pt 2>&1 | tee logs/reacher_arms_${NAME}_seed${SEED}.log")
    done
  done
  printf '%s\n' "${cmds[@]}" | fan_out
}
