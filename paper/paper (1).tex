...
success-rate-neutral at the official criterion. On a second environment (Reacher,
exploratory non-expert data), this consumption mechanism transfers with zero re-tuning
--- SR-neutral at every horizon, reaching $95.5\%$ at ${\sim}20\times$ less drafter
compute at the strongest cell --- while drafting itself proves regime-dependent:
against the strongest flat baseline at every replanning rate, drafted subgoals win
$+11$ to $+19$pp exactly where the goal outruns the planning window, and are a
measurable detour tax where it does not. Refinement heads, learned confidence
...
fixed to isolate the former, and the mirror arm (every-step at $k{=}8$, isolating the
consumption policy against an equally-cheap-per-call baseline) is \TBD: run in flight.
...
every-step drafting, $k{=}8$ (cheap-call mirror)   & \TBD & \TBD & 1.00 & 240 \\
...
ways. First, a re-selection of $\tau$ on an episode-disjoint validation population
(selection on validation, chosen cell reported on the untouched evaluation split) is
\TBD: run in flight. Second, and independently, the same $\tau{=}0.20,k{=}8$ transferred
to Reacher with \emph{zero re-tuning} and remained SR-neutral at call ratios
$0.59$--$0.88$ in every cell of every regime (\Cref{sec:reacher}) --- the behavior of a
...
(Remaining cells of this grid, and an in-population flat arm for \Cref{tab:horizon},
are \TBD: runs in flight.)
...
goal-free control drafter quantifies this claim: \TBD\ (run in flight).
...
drafter curve sits far below flat planning (\Cref{tab:reacher-native}). Two controls
attribute this. First, an \emph{oracle} arm (ground-truth dataset latents as subgoals)
also fails to beat flat planning ($90.6$ vs.\ $93.8$, $n{=}32$): a single short reach
inside the planner's lookahead gains nothing from decomposition, bounding all drafters
at parity. Second, the deficit below that bound is a \emph{detour tax}: waypoints
...
merely under-specified by a remote goal image.
...
frontier: $k{=}4$ dominates uniformly across $\tau$, and at $\tau{=}0.2,k{=}4$
spec-accept reaches $95.5\%$ ($n{=}4$ seeds) at $2.6$ drafter NFE per replan ---
\ours{${\sim}20\times$ below} the every-step reference's 50 NFE, while \emph{exceeding}
its SR ($90.8$). SR degrades gracefully with $\tau$, as an acceptance threshold should.
...
$t{=}150$ cell is a beyond-paper probe, not a reproduction claim. (v) PushT results are
being re-derived end-to-end from public assets on independent hardware at the time of
writing; the re-derivation gates (drafter-diagnostic and anchor-cell reproduction) are
\TBD\ and will be reported.
