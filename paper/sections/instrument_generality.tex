...
are frozen and running (\Cref{sec:generality-dinowm}).
...
\inflight{P1--P5 batteries running at submission-draft time (12 + 12 + 8 arms,
seeds 99--102, $n{=}50$/cell); this paragraph reports frozen predictions, and the
camera-ready inserts verdicts as measured. Point-maze remains prediction-only (its
environment requires mujoco-py + d4rl).}
...
\inflight{The closed-loop completion, a DINO-WM world model trained on TwoRoom
(running), a drafted arm under the lens verifier, bars frozen on model completion, lands after the draft freeze and is reported as measured.}
