...
replan. One mechanism explains the wins, the ties, and every measured negative:
...
population effect ($+14.6$pp). \textbf{(iii) Verifier modularity recovers
operability.} Certifying in a frozen general-purpose encoder (DINOv2) while the
planner keeps consuming LeWM latents restores a working accept test, a single
drafter emits both halves of each waypoint jointly so planner and verifier
describe the same imagined future, and at range the certified loop clears flat
($43.4 \to 48.8$ at $t{=}75$, twelve seeds, certified protocol). Oracle headroom
is $+8.6$pp at $t{=}75$ against $-18.2$pp at $t{=}25$, so TwoRoom is a
horizon-extension case, as PushT and Reacher already are.
\inflight{The pre-registered re-measurement at the anchored protocol (flat swept
over replan rate, an oracle-headroom gate, and best-of-$k$ drafting scored in the
verification space) is running at draft time; bars were frozen before any cell
ran.}
...
the rule was frozen. Nothing in the recipe is substrate-specific: any world model
that plans by rollout can grade its own homework the same way, and the open frontier
is scale, where the drafter tier rather than the controller is the wall-clock.
