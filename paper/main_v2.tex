...
built from one training-free rule and a set of measured guarantees about that rule.
The rule is \emph{spec-accept}: draft a block of $N$ subgoal latents once, then at
every replan boundary verify the \emph{achieved} latent against the waypoint just
pursued; within a relative distance $\tau$ the next pre-drafted waypoint is served
at zero drafter cost, otherwise the block is re-drafted from reality. Every
constant is derived offline rather than tuned: $\tau$ is the encoder's own
criterion floor, the step budget $k$ is the sampler's convergence point. The
guarantees are then earned the hard way. Calibrated against ground-truth state,
the accept test tracks physical truth (Spearman up to $0.98$; false-accepts
quantified per environment and shown to be near-boundary). Under injected drafter
...
ourselves shows the test's per-event selectivity is \emph{not} what carries
success at nominal operation: the verifier's closed-loop value is discovering,
adapting, and certifying the re-anchoring \emph{rate}. A companion statistic, the
verification gap, predicts from cached latents alone whether a substrate can
support the method at all; its verdicts have held prospectively on every
substrate adopted after the rule was frozen, including a $31$pp cross-architecture
failure called in advance. Validated on its substrate's entire published suite,
certified spec-accept meets or beats the strongest flat baseline in all eleven
pre-registered environment--horizon cells (margins up to $+94$pp where flat
planning collapses) at $1.9$--$2.4$ drafter evaluations per replan; on the one
environment whose encoder fails the gap precondition (Two-Room, the substrate
authors' own weakest result) the accept test transplants unchanged into a frozen
external encoder and recovers a $+15.4$pp certified win at long horizon, with a
window rule whose switch is read off the measured crossover composing the two
regimes into a policy that never loses to flat.
\end{abstract}
...
ground-truth calibration table (false-accept and false-reject rates at each
environment's own success radius, with the residual errors shown near-boundary), a
corruption sweep in which closed-loop rejection ignites exactly at the
offline-derived $\tau$, and a pre-registered matched-rate random-rejection control
that \emph{refutes} the strongest reading of our own mechanism: per-event
selectivity is not load-bearing at nominal operation. What survives, with
dose-response evidence, is sharper: verification is a calibrated, adaptive
controller of the re-anchoring rate, whose fixed-rate shadow is knowable only in
hindsight and cannot ignite under divergence.
...
certified arrival; the resulting single policy wins all eleven pre-registered
environment--horizon cells of the substrate's published suite, our primary
validation of the layer as a whole.
...
\item \textbf{A pre-flight applicability instrument and honest scope}
(\Cref{sec:why-regime,sec:discussion}): the verification gap predicts from cached
latents whether a substrate supports the method; its verdicts held prospectively on
every substrate adopted after the rule was frozen (including a $31$pp
cross-architecture failure called before the run), and every boundary of the
derivation is measured and reported alongside the retained negative results.
\end{enumerate}
...
The word \emph{certified} is cheap unless the certificate itself is audited. This
subsection does that, pre-registered end to end: what the accept test's decisions
mean physically (calibration), whether the derived $\tau$ has closed-loop content
(corruption), and which part of the decision actually carries success (a control we
ran against our own mechanism).

\textbf{Calibration against ground truth.} For every headline serving arm we
replayed each verification event offline (replay mismatches: zero everywhere) and
decoded both the achieved latent and the pursued waypoint to ground-truth state
through a probe trained on dataset frames, gated at held-out $R^{2}\geq0.90$ on the
criterion dimensions with a pre-frozen conservative-row rule when probe classes
disagree. \Cref{tab:calibration} gives the operating characteristic at the derived
$\tau$, against each environment's \emph{own success radius}. Accepted waypoints are
physically within the task's success radius $100\%$/$90\%$ of the time on
Cube/PushT; on the two tightest criteria (Reacher $0.05$\,rad, Two-Room $16$\,px)
accepts sit at $80$--$107\%$ of the radius with $2.5$--$3\times$ separation from
rejects, and Reacher's verifier ranks true angular distance at $\rho=0.978$, a
near-perfect ordinal sensor. The residual false-accepts are near-boundary, not
gross: sweeping the readout radius (decisions and $\tau$ fixed), Reacher's
false-accept rate falls $0.400\to0.064$ at twice the criterion radius and $0.003$
at three times; Two-Room's falls $0.522\to0.130$ at twice. Errors elsewhere are
conservative (high false-reject, low false-accept): the verifier errs by paying an
unnecessary re-draft, not by serving a bad waypoint.
...
drafted waypoint by relative noise $\sigma$ (at every draft, the deployed
semantics) and serving through the unchanged verifier, the call ratio ignites from
its banked baseline to total rejection exactly as $\sigma$ crosses the derived
threshold: $0.59\to0.66\to1.00\to1.00$ across $\sigma=0/0.1/0.2/0.4$ (free
advances $518\to420\to2\to0$), the ignition point our offline derivation predicted
before the sweep ran. The companion prediction failed honestly and is reported:
success itself was flat for both verified and blind arms at every $\sigma$,
because fresh-draw isotropic jitter is temporally averaged by \emph{any}
consumption policy; verification's success-rate value is against
\emph{autocorrelated} divergence (\Cref{sec:why-verifier}), not white noise.

\textbf{What the decision carries: rate, not per-event selectivity.} The sharpest
question a certificate must survive is whether its decisions matter at all. We
pre-registered a matched-rate random-rejection control: replace the accept test
with an i.i.d.\ coin that rejects at the banked cells' own measured per-event
rejection probability ($0.494$/$0.490$, reconstructed exactly from serving-log
counters), everything else byte-identical. The coin \emph{ties} certified
spec-accept on both $t{=}150$ headline cells ($98.05$ vs.\ $98.14$ on PushT;
$90.33$ vs.\ $91.11$ on Reacher, the residual $+0.78$pp consistent across seeds
but below our $3$pp bar). A pre-registered dose-response below the matched rate
completes the decomposition: success climbs monotonically with rejection rate on
Reacher ($84.6$ at $p{=}0$, the blind floor, to $90.4$ by $p{=}0.25$) and is a
step function on PushT (blind commitment loses $18$pp; $p{=}0.125$ already
recovers to within $1.2$pp). At nominal operation, then, \emph{which} replans are
rejected is not load-bearing; \emph{how often} is. Three things the coin cannot
do keep the verifier essential, and each is measured above: its matched rate
exists only in hindsight, read off the verifier's own telemetry; a fixed coin
cannot ignite when reality diverges (the corruption sweep); and its decisions
certify nothing (the calibration table is a property of the latent test, not of a
schedule). Verification, on this evidence, is a \emph{calibrated, adaptive
controller of the re-anchoring rate}, and the paper's mechanism claims are stated
at that strength and no more.
...
cell ($1{,}526$ log rows), the call ratio falls with goal distance exactly as the
regime map predicts: PushT $0.88\to0.57$ and Reacher $0.75\to0.58$ across
$t{=}25\to150$, with Two-Room's $0.96$ the registered weak-encoder exception. The
verifier's telemetry is thus a free, per-episode regime diagnostic, the same
signal the $c^{*}$ arbiter reads at episode scope.

\textbf{The same audit runs at foundation scale.} The calibration methodology is
not tied to its substrate. Applied to V-JEPA 2 ViT-g pooled latents over $400$
recorded DROID episodes, pre-registered as a \emph{dataset-pair} characteristic
(no serving loop exists on that stack; both frames of every pair are real, so
ground truth is recorded rather than probed), the transferred $\tau{=}0.20$
accepts $85\%$ of within-episode pairs across the far-goal horizon, rejects
$99.9\%$ of cross-episode pairs, and the accept statistic rank-tracks
end-effector distance at $\rho=0.53$, above PushT's own deployed row. The
threshold is metrically \emph{loose} there (median accepted pair $=12.7$\,cm of
end-effector motion; DROID defines no task radius), so the row certifies rank
sensitivity and scene discrimination rather than a success-radius guarantee,
exactly the distinction this table is designed to expose.
...
% ============================================================
\input{sections/instrument_generality}
