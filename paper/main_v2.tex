...
\begin{abstract}
Hierarchical latent world-model planners decompose a goal into drafted subgoal
latents steered by a flat controller, paying one diffusion call per replan and
committing to a subgoal spacing fixed by convention. We ask the method question these
systems leave open: when can a drafted plan be \emph{trusted}, and for how long? We
introduce \emph{spec-accept}, a training-free consumption rule that drafts a block of
$N$ waypoints once and, at each replan boundary, verifies reality against the plan ---
if the achieved latent is within a relative distance $\tau$ of the waypoint just
pursued, the next pre-drafted waypoint is served at zero drafter cost; otherwise the
block is re-drafted from the achieved state. The verifier is the environment itself:
no learned confidence head, no schedule, no new parameters. Its threshold $\tau$ and
the drafter's step budget $k$ are not tuned but \emph{derived} from two offline
measurements of the stack --- the encoder's criterion floor and the sampler's
convergence point --- computed in minutes; on PushT the derived $k{=}3$ was never
swept and was confirmed prospectively. Calibrated on two development environments and
applied unchanged, the recipe beats the published FF-JEPA system on its own protocols
($96.1{\to}99.4$ short horizon, $82.4{\to}100.0$ random initialization) and beats
flat planning by $+95.7$/$+19.7$pp at long horizon --- where flat planning collapses
with goal distance --- while spec-accept holds success-rate-neutral at $1.9$--$2.4$
drafter NFE per replan (the inherited $50$-step sampler was convention, not
necessity). One mechanism explains the results: closed-loop control pays for target
consistency, re-drafting injects churn proportional to draft noise, and verification
is regime-dependent but predictable --- idle where drafts track reality (expert data),
load-bearing where they diverge ($+5$--$7$pp over blind commitment on exploratory
data), with the realized call ratio a free per-episode diagnostic of which regime
holds. The regime rule is then \emph{operationalized} by the same machinery: one flat
CEM probe --- the planner's own predicted terminal discrepancy, $c^{*}$, thresholded
at the same $\tau$ --- certifies per episode and per replan whether drafting is needed
at all, and the resulting policy, \emph{certified spec-accept}, meets or beats the
strongest flat configuration at \emph{every} horizon on all three environments (pre-registered
confirmations on fresh populations: $4/4$ Reacher cells, $+10.5$pp over flat on
held-out Cube, $5/5$ do-no-harm cells on PushT), at zero drafter cost on
certified-easy episodes. We report the derivation's scope honestly: it is validated
retrospectively on two environments with one prospective confirmation, and its
criterion-floor threshold rule has a \emph{measured} boundary --- tasks whose success
criterion covers only part of the observed scene inflate the floor and need a
different threshold basis.
\end{abstract}
...
fixed stride. The system works --- hierarchy beats flat planning at long horizon ---
but two questions go unexamined. It pays one diffusion call at \emph{every} replan,
...
possible verifier, the achieved environment state, is free at every replan boundary
--- and to set the two knobs this introduces by \emph{measurement} rather than search.
Concretely:

\begin{enumerate}[itemsep=2pt,topsep=2pt]
\item \textbf{Spec-accept} (\Cref{sec:method-spec}): a training-free consumption rule
that drafts an $N$-block once and re-drafts only when the achieved latent leaves a
tolerance $\tau$ of the waypoint being pursued --- one threshold, no learned
components, composes with any drafter.
\item \textbf{Calibration by measurement} (\Cref{sec:calib}): $\tau$ is the encoder's
criterion floor and $k$ (the drafter's step budget) is the sampler's convergence
point, each read off an offline measurement in minutes rather than a closed-loop
sweep. On PushT the derived $k{=}3$ was a configuration the sweep never ran, predicted
and then confirmed; $S$ (subgoal scale) remains the one tuned knob and its interior
optimum is reported as a finding.
\item \textbf{Evaluation and mechanism} (\Cref{sec:results,sec:why}): the recipe is
calibrated on two development environments and applied unchanged, with every
comparison anchored to the strongest flat baseline per cell. A single mechanism --- the
\emph{churn thesis}: control pays for target consistency, re-drafting injects churn
proportional to draft noise --- unifies five measurements and makes verification's
value \emph{regime-dependent but predictable}.
\item \textbf{Certified drafting scope} (\Cref{sec:method-arbiter,sec:results-unified}):
the regime characterization is turned into a \emph{policy} by the planner's own
reachability certificate $c^{*}$ --- one flat CEM probe, thresholded at the same
$\tau$ --- which routes each episode to flat planning or drafting at its start and
retires the drafter on certified arrival. The resulting policy, \emph{certified
spec-accept}, meets or beats the strongest per-cell baseline at every horizon on all
three environments, confirmed pre-registered on fresh populations.
\item \textbf{Honest scope} (\Cref{sec:discussion}): the derivation is validated
retrospectively on two environments with one prospective hit; we report a measured
boundary of the criterion-floor rule and a battery of negative results that
...
literally to latent control, every learned component hurts (\Cref{sec:negatives}) ---
the skeleton survives only once the learned verifier is replaced by the achieved
...
\citep{spo} speculatively executes cloud policy outputs to mask network latency ---
speculation over transport, not over drafter compute.
...
\TBD{}: re-run the concurrent-work sweep at submission time (last swept 2026-07-13;
closest neighbors then: \citep{spo,bid,rtc,dcdp}).
...
pursued, the next pre-drafted waypoint is served at zero drafter cost; otherwise the
block is re-drafted from the achieved state. Re-anchoring is never sacrificed ---
drafting is skipped only while reality confirms the plan. The verifier is the
...
corollary test: the band's upper edge should scale with the real-divergence scale
--- coarser $S$ means larger per-hop tracking error, so looser $\tau$ should stay
...
$2{\to}3$, predicting $k{=}3$ lands on the plateau; run afterwards, it does ---
every-step $63.3{\pm}0.5$, spec-accept $64.5{\pm}1.1$ at call ratio $0.63$
...
inferability leg); TwoRoom is reported as a measured boundary case (unobservable
goal), not a battery.
...
shown. \TBD{}: third panel (Cube, \Cref{tab:cube}) --- plotting task.}
\label{fig:spine}
\end{figure}
...
ran, failures reported as-is.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{3.2pt}
\caption{\textbf{The headline grid.} \emph{Certified spec-accept} (spec-accept $+$
the $c^{*}$ arbiter) vs.\ the strongest flat configuration per cell and our own fixed
spec-accept arm, on the confirmation populations at $1024$ episodes/cell (PushT
$4$ seeds $\times\,n{=}256$; Reacher $8$ seeds $\times\,n{=}128$, fresh
builder-seed-$999$ population; Cube $8$ fresh seeds $\times\,n{=}128$, certified
protocol). The fixed spec-accept arm is each environment's certified configuration
(PushT $k{=}3$ goal-free; Reacher $k{=}8$ goal-conditioned; Cube $k{=}3$
goal-conditioned $+$ arrival gate). Pre-registered verdicts on the declared seeds:
Reacher $4/4$, PushT $5/5$, Cube $+14.0$ against a $+5$ bar. Bold $=$ best in
column. Cube contributes one cell by construction, not omission: cube-single is a
single-transport task (delivery then idle), so earlier offsets start after the cube
has already arrived and are vacuous --- offset $150$ is the one cell that is the
published task, anchored against LeWM's $74\%$ (\Cref{sec:results-heldout});
multi-cube is the true long-horizon manipulation case and requires unreleased data.
Certified spec-accept beats the strongest flat arm in \emph{all eleven cells};
every-step reference arms and population definitions in
...
multiply into any stack with a diffusion proposer, independent of what surrounds it
--- including stacks whose controller has been distilled or amortized away, where
...
\emph{fastest} and the most successful arm ($1.62$s/episode vs.\ flat's $2.11$) ---
routing sends half the episodes to the cheap short window, and successful episodes
...
Reacher. \textbf{Middle:} what it buys. \textbf{Right:} time per completed task ---
the failing baselines invert from cheapest to most expensive.}
\label{fig:cost}
\end{figure}
...
The structurally hardest case for drafting, reported as a measured boundary case
rather than a battery: the per-episode target is randomized and not rendered in the
observation --- the sharpest form of the regime rule's inferability leg
(\Cref{sec:why-regime}) --- so goal-free drafting is ill-posed by construction, and
even goal-conditioned preliminary runs sat well below dev-env fidelity. Consistent
with the rule, and honestly outside the recipe's certified scope.
...
the recipe at the transferred-default $\tau$ with the (in-scope) derived $k$ ---
which, applied derive-first to the cube drafter, named $k{=}3$; the closed-loop sweep
...
$0.14$--$0.18$) although the physical transport far exceeds the planning window ---
under a partial-observation criterion the certificate inherits the planner's optimism
...
commitment scores $87.9/89.8$ at $t{=}100/150$ vs.\ spec-accept's $95.5/95.7$ ---
a $+5.9$--$7.6$pp gap. The rejections are re-anchoring events, exactly where the
...
anomaly (spec-accept \emph{beating} its own every-step reference by $+5$pp ---
skipping verified re-drafts of a close target is better control), the $k$-cliff
...
TwoRoom (goal not inferable $\to$ drafting fails, predicted); Cube --- the honest
...
TwoRoom & yes & \textbf{no} & yes & drafting fails --- predicted \\
...
\Cref{sec:why-verifier} complete the picture: blindness is not uniformly wrong ---
it ties where verification is idle --- but it fails unsignaled in both directions
...
Hierarchical latent planners can trust a drafted plan exactly as long as reality
confirms it, and the threshold for ``confirms'' is not a hyperparameter but a measured
property of the encoder. Spec-accept turns this into a training-free consumption rule
with one threshold; calibration by measurement removes the closed-loop sweep that would
otherwise price its generality; and a single mechanism --- control pays for target
consistency, re-drafting injects churn proportional to draft noise --- explains why the
recipe beats the published system and flat planning at long horizon, why verification
is idle on expert data and load-bearing on exploratory data, and why every operator
that narrows the drafter's spread loses. On two development environments the recipe is
SR-neutral-or-better at $1.9$--$2.4$ drafter NFE per replan with zero re-tuning, and
its one derived configuration was, in the sharpest test available, a point the sweep
never visited --- twice, once per environment family, after Cube's derive-first
$k{=}3$ also proved the sweep's strongest cell. The regime-dependence of drafting,
this paper's central empirical finding, is then closed by its own instrument: the
planner's reachability certificate arbitrates flat planning against drafting per
episode and per replan, and the resulting single policy --- certified spec-accept ---
meets or beats the strongest per-cell baseline at every horizon on all three
environments, confirmed pre-registered on fresh populations. Nothing in the recipe is substrate-specific ---
any world model that plans by rollout can grade its own homework the same way --- and
the open frontier is scale: the same consumption policy and the same certificate,
applied to a video-scale world model where the drafter tier, not the controller,
is the wall-clock.
...
against \emph{unchanged} $25$-step targets collapses SR to $78.6/77.8$ ($-12.5$pp) ---
the mechanism is scale matching, not replan frequency.
