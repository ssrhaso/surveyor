"""TwoRoom criterion floor in the LeWM latent space, for the ARBITER's probe.

Battery TR (2026-09-24). On Two-Room the accept rule checks in the DINOv2 half of the paired drafter's latent, and its
tolerance (0.127) was measured there by probe_dino_gap.py. The arbiter's probe c* is different: it is the predicted
terminal latent of a flat CEM plan on the frozen LeWM, compared with the LeWM goal latent, so a tolerance for it must be
measured in LeWM space. This file is probe_dino_gap.py with ONE change, the encoder (encoder.encode_frames on the frozen
LeWM, the exact path the eval uses for obs and goal latents); the episode sample, the pair populations, the seeds and
the bootstrap are identical, so the two tolerances are measured on the same pairs. Generated by make_lewm_probe.py.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="TwoRoom LeWM-space criterion floor (arbiter probe tolerance)")
    ...
    p.add_argument("--out", default="Results/gap_stat/gap_tworoom_lewm.json")
...
def main():
    ...
    model = encoder.load_lewm(source=args.source, encoder_id=args.encoder_id, local_dir=args.local_dir,
                              swm_src=args.swm_src, device=dev)
    model.eval()
    print(f"[probe] frozen LeWM {args.encoder_id} loaded on {dev} (training={model.training})")
    ...
    with h5py.File(args.h5, "r") as f:
        ...
        for k, e in enumerate(eps):
            off, L = int(ep_off[e]), int(ep_len[e])
            z = encoder.encode_frames(model, pixels[off:off + L], device=dev)
            zn = z.numpy()
            lat.append(zn)
    ...
    out = {"name": "tworoom-lewm", "encoder_id": args.encoder_id, "h5": args.h5,
