...
def parse_args():
    p = argparse.ArgumentParser(description="FF-JEPA Task A (Cube): subgoal-dataset builder")
    p.add_argument("--h5", required=True, help="cube_single_expert.h5 path")
    ...
    p.add_argument("--stride", type=int, default=1,
                   help="frame subsample stride; 1 (dense) so train_gdm --subgoal-step S "
                        "can carve any stride from one file")
...
def main():
    ...
    with h5py.File(args.h5, "r") as f:
        ...
        # expert data: every episode is a successful demo -> no filter
        kept_rows_per_ep, kept_ids, kept_len = [], [], []
        for e in ep_ids:
    ...
    out = {
        ...
        "in_5deg": torch.ones(n_kept, dtype=torch.bool),   # no success tiers; train_gdm --mask compat
        "stride": args.stride,
        "latent_dim": int(D),
        "criterion": {"mode": "cube_expert_no_filter"},
        ...
        "counts": {"episodes_considered": int(len(ep_ids)),
                   ...
                   "kept_headline": int(n_kept), "dropped": 0},
